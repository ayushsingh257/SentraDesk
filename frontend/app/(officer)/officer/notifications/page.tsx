...
export default function OfficerNotifications() {
  ...
  const handleNotificationClick = async (notif: InAppNotification) => {
    ...
    if (notif.ticket_id) {
      router.push(`/officer/tickets/${notif.ticket_id}`)
    }
  }
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-neutral-200 dark:border-neutral-800 pb-5">
        <div>
          ...
          <p className="mt-1.5 text-sm text-neutral-500 dark:text-neutral-400">
            You have {unreadCount} unread case updates and system alerts.
          </p>
        </div>
      ...
      <Card className="p-0 overflow-hidden border border-neutral-200/80 dark:border-neutral-800 shadow-card">
        {notifications.length === 0 ? (
          <div className="py-16 text-center space-y-3">
            <Bell size={40} className="text-neutral-350 dark:text-neutral-600 mx-auto" />
            <h3 className="text-sm font-bold text-neutral-800 dark:text-neutral-250">No notifications found</h3>
            <p className="text-xs text-neutral-450 max-w-xs mx-auto">
              Any alerts regarding new case assignments, citizen uploads, or supervisor responses will appear here.
            </p>
          </div>
        ) : (
